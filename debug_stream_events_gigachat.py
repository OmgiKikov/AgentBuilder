...
# Импортируем GigaChat модель
from graph.helpers.gigachat_provider import GigaChatModel
    ...
    def on_span_start(self, span):
        ...
        try:
            ...
            if span.span_data.type == "agent":
                ...
                print(f"{indent}  Model: {data.get('model', 'GigaChat')}")
...
async def run_streamed(
    ...
    """
    Wrapper function for initializing and running the agent with GigaChat in streaming mode.
    """
    print(f"Initializing GigaChat streaming client for agent: {agent.name}")
    ...
    # Format messages to ensure they're compatible with the GigaChat API
    formatted_messages = []
    for msg in messages:
    ...
    print("Beginning GigaChat streaming run")
    ...
    try:
        ...
        if enable_tracing:
            ...
            async def wrapped_stream_events():
                ...
                with trace(f"GigaChat Agent turn: {agent.name}") as trace_ctx:
                    try:
                        ...
                        print(f"Error in GigaChat stream events: {str(e)}")
                        raise
        ...
        print(f"Error during GigaChat streaming run: {str(e)}")
        raise

# Создаем простой внешний инструмент для GigaChat
async def simple_tool():
    """simple_tool который выдает ответ на русском языке"""
    return "Бим Бам Бом"

# Создаем FunctionTool для агента
simple_function_tool = FunctionTool(
    name="simple_tool",
    description="Простой инструмент который выдает приветственный ответ на русском языке",
    params_json_schema={
        "type": "object",
        "properties": {},
        "required": []
...
# Создаем агента с GigaChat моделью
try:
    print("🔧 Инициализируем GigaChat модель...")
    gigachat_model = GigaChatModel()
    
    agent = NewAgent(
        name="debug_gigachat_agent",
        model=gigachat_model,
        instructions="Ты полезный помощник на базе GigaChat. Отвечай на русском языке. Используй доступные инструменты когда это необходимо.",
    ...
    print("✅ GigaChat агент создан успешно!")
    
except Exception as e:
    print(f"❌ Ошибка создания GigaChat агента: {e}")
    agent = None
...
async def debug_stream_events():
    print("=== Начинаем отладку stream_events для GigaChat ===")
    
    if agent is None:
        print("❌ Агент не создан, завершаем выполнение")
        return
    
    # Запускаем swarm_run_streamed с GigaChat
    ...
    print("=== Обрабатываем события GigaChat ===")
    
    # Обрабатываем события
    async for event in stream_result.stream_events():
        print(f"\n--- GigaChat Событие: {event.type} ---")
        ...
        if hasattr(event, 'item'):
            ...
            # Специальная обработка для GigaChat текстового контента
            if hasattr(event.item, 'type') and event.item.type == "message_output_item":
                if hasattr(event.item, 'raw_item') and hasattr(event.item.raw_item, 'content'):
                    print("📝 GigaChat текстовый контент:")
                    for content_part in event.item.raw_item.content:
                        if hasattr(content_part, 'text'):
                            print(f"   Текст: {content_part.text}")
        
        print("-" * 50)

if __name__ == "__main__":
    print("🔥 Отладка stream_events для GigaChat")
    print("=" * 60)
