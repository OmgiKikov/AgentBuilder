...
import requests
from .schema import ApiRequest, ApiResponse, ApiMessage, UserMessage, AssistantMessage, AssistantMessageWithToolCalls


class Client:
    def __init__(self, host: str, project_id: str, api_key: str) -> None:
        self.base_url: str = f"{host}/api/v1/{project_id}/chat"
        self.headers: Dict[str, str] = {"Content-Type": "application/json", "Authorization": f"Bearer {api_key}"}
    ...
    def chat(
        ...
        response_data = self._call_api(
            messages=messages, state=state, workflow_id=workflow_id, test_profile_id=test_profile_id
        )
...
class StatefulChat:
    ...
    def __init__(
        self, client: Client, workflow_id: Optional[str] = None, test_profile_id: Optional[str] = None
    ) -> None:
        self.client = client
    ...
    def run(self, message: Union[str]) -> str:
        ...
        response_data = self.client.chat(
            messages=self.messages, state=self.state, workflow_id=self.workflow_id, test_profile_id=self.test_profile_id
        )
...
if __name__ == "__main__":
    ...
    result = client.chat(messages=[UserMessage(role="user", content="Hello")])
