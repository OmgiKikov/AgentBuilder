#!/usr/bin/env python3
"""
Автоматическое обновление REAL_WORKFLOW_ID в designtime.json
"""
...
def auto_update_designtime_workflows(filepath="designtime.json"):
    """
    Автоматически обновляет REAL_WORKFLOW_ID в designtime.json
    """
    print("🔄 Автоматическое обновление designtime.json")
    print("=" * 45)
    
    # Загружаем designtime.json
    try:
        with open(filepath, 'r', encoding='utf-8') as f:
            designtime_data = json.load(f)
    ...
    print(f"📊 Найдено тестовых сценариев: {len(designtime_data)}")
    ...
    for i, scenario in enumerate(designtime_data):
    ...
    if updated_count > 0:
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            json.dump(designtime_data, f, indent=4, ensure_ascii=False)
...
if __name__ == "__main__":
    filepath = "benchmark/design_time/designtime.json"
    auto_update_designtime_workflows(filepath) 