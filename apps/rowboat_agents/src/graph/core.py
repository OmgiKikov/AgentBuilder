...
import uuid
import logging
from .helpers.access import get_agent_by_name, get_external_tools, get_prompt_by_type, get_agent_config_by_name
    ...
    async def stream(
        ...
        try:
            ...
            new_agents = get_agents(
                agent_configs=agent_configs, tool_configs=tool_configs, complete_request=complete_request
            )
                    ...
                    try:
                        ...
                        # Handle agent transfer
                            ...
                            if current_count >= event.new_agent.max_calls_per_parent_agent:
                                print(
                                    f"Skipping transfer from {current_agent.name} to {event.new_agent.name} (max calls reached from parent to child)"
                                )
                                continue
                                ...
                                "tool_calls": [
                                    {
                                        "function": {
                                            ...
                                            "arguments": json.dumps({"assistant": event.new_agent.name}),
                                        },
                                        "id": tool_call_id,
                                        "type": "function",
                                    }
                            ...
                            message = {
                                "content": json.dumps({"assistant": event.new_agent.name}),
                                "role": "tool",
                                "sender": None,
                                "tool_calls": None,
                        ...
                        # Handle regular messages and tool calls
                            ...
                            if event.item.type == "tool_call_item":
                                ...
                                if (
                                    hasattr(event.item.raw_item, "type")
                                    and event.item.raw_item.type == "web_search_call"
                                ):
                                    web_search_messages = handle_web_search_event(event, current_agent)
                                    for message in web_search_messages:
                                        message["response_type"] = ResponseType.INTERNAL.value
                                            ...
                                            for annotation in content_item.annotations:
                                                if hasattr(annotation, "type") and annotation.type == "url_citation":
                                                    citation = {
                                                        ...
                                                        "title": (
                                                            annotation.title if hasattr(annotation, "title") else ""
                                                        ),
                                                        "start_index": (
                                ...
                                response_type = (
                                    ResponseType.INTERNAL.value if is_internal else ResponseType.EXTERNAL.value
                                )
                                ...
                                if is_internal and parent_stack:
                                        ...
                                        "tool_calls": [
                                            {
                                                "function": {
                                                    "name": "transfer_to_agent",
                                                    "arguments": json.dumps({"assistant": parent_stack[-1].name}),
                                                },
                                                "id": tool_call_id,
                                                "type": "function",
                                            }
                                    ...
                                    transition_response = {
                                        "content": json.dumps({"assistant": parent_stack[-1].name}),
                                        "role": "tool",
                                        "sender": None,
                                        "tool_calls": None,
                                        "tool_call_id": tool_call_id,
