...
def get_last_agent_name(
    state, agent_configs, start_agent_name, msg_type, latest_assistant_msg, start_turn_with_start_agent
):
    ...
    if msg_type == "tool":
        ...
        assert last_agent_name == latest_assistant_msg.get(
            "sender", ""
        ), "Last agent name does not match sender of latest assistant message during tool call handling"
