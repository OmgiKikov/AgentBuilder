        ...
        if event.type == "raw_response_event":
            if hasattr(event, "data") and hasattr(event.data, "raw_item"):
                ...
                if (hasattr(raw_item, "type") and raw_item.type == "web_search_call") or (
                        ...
                        "tool_calls": [
                            {
                                "function": {
                                    "name": "web_search",
                                    "arguments": json.dumps({"search_id": call_id, "status": status}),
                                },
                                "id": call_id,
                                "type": "function",
                            }
                    ...
                    print(
                        f"Condition for tool call matched in raw_response_event. Appending tool call message: {tool_call_msg}"
                    )
        ...
        # Handle run item web search events
            ...
            if event.item.type == "tool_call_item":
                try:
                    ...
                    if hasattr(event.item.raw_item, "type") and event.item.raw_item.type == "web_search_call":
                        ...
                        tool_call_msg = {
                            ...
                            "tool_calls": [
                                {
                                    "function": {"name": "web_search", "arguments": json.dumps({"search_id": call_id})},
                                    "id": call_id,
                                    "type": "function",
                                }
                            ],
                        ...
                        print(
                            f"Condition for tool call matched in run_item_stream_event. Appending tool call message: {tool_call_msg}"
                        )
                        messages.append(tool_call_msg)
                ...
                if isinstance(event.item.raw_item, dict) and event.item.raw_item.get("type") == "web_search_results":
                    ...
                    print(
                        f"Condition for tool call output matched in run_item_stream_event. Appending tool call output message: {tool_call_output_msg}"
                    )
                    messages.append(tool_call_output_msg)

            elif event.item.type == "web_search_call_item" or (
                hasattr(event.item, "raw_item")
                and hasattr(event.item.raw_item, "type")
                ...
                hasattr(event.item, "raw_item")
                and (
                    (hasattr(event.item.raw_item, "type") and event.item.raw_item.type == "web_search_results")
                    or (
                        isinstance(event.item.raw_item, dict)
                        and event.item.raw_item.get("type") == "web_search_results"
                    )
                )
            ):
