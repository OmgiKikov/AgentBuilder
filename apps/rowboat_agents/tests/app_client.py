...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sample_request", type=str, required=True, help="Sample request JSON file name under tests/sample_requests/"
    )
    parser.add_argument("--api_key", type=str, required=True, help="API key to use for authentication")
    parser.add_argument(
        "--host", type=str, required=False, help="Host to use for the request", default="http://localhost:4040"
    )
    ...
    response = requests.post(
        f"{args.host}/chat", json=request, headers={"Authorization": f"Bearer {args.api_key}"}
    ).json()
    print("Output: ")
    print(response)
