...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", type=str, required=False, default="default_config.json", help="Config file name under configs/"
    )
    parser.add_argument(
        "--sample_request",
        type=str,
        required=False,
        default="default_example.json",
        help="Sample request JSON file name under tests/sample_requests/",
    )
    parser.add_argument("--api_key", type=str, required=False, default="test", help="API key to use for authentication")
    parser.add_argument("--host", type=str, default="http://localhost:4040", help="Host to use for the request")
    parser.add_argument("--load_messages", action="store_true", help="Load messages from sample request file")
    ...
    while True:
        ...
        resp_messages, resp_state = stream_chat(host=args.host, request_data=complete_request, api_key=args.api_key)
