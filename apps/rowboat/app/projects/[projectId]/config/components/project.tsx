...
function ProjectNameSection({ projectId }: { projectId: string }) {
    ...
    return <Section 
        title="Имя проекта"
        description="Имя вашего проекта."
    >
        {loading ? (
            <Spinner size="sm" />
        ) : (
...
function ProjectIdSection({ projectId }: { projectId: string }) {
    return <Section 
        title="ID проекта"
        description="Уникальный идентификатор вашего проекта."
    >
...
function SecretSection({ projectId }: { projectId: string }) {
    ...
    return <Section 
        title="Secret ключ проекта"
        description="Secret ключ проекта используется для подписи запросов инструментов, отправляемых в ваш webhook."
    >
            ...
            {loading ? (
                ...
                <div className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
                    <div className="grid grid-cols-12 items-center border-b border-gray-200 dark:border-gray-700 p-4">
                        ...
                        <div className="col-span-2 font-medium text-gray-900 dark:text-gray-100">Последнее использование</div>
                    </div>
...
<script>
    ...
    return (
        <Section 
            title="Чат виджет"
            description="Добавьте чат виджет на ваш сайт, скопировав и вставив этот код сниппет прямо перед закрывающим тегом </body>."
        >
            <div className="space-y-4">
                {loading ? (
                    <Spinner size="sm" />
...
function DeleteProjectSection({ projectId }: { projectId: string }) {
    ...
    return (
        <Section 
            ...
            description="Удалить этот проект и все его данные."
        >
            <div className="space-y-4">
